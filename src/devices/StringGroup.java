...
//group of stringsenders
public class StringGroup implements Programmable, Group {
	...
	public void add(Programmable member) throws IncompatibleProtocolException, IllegalOperationException {
		if (!member.getDevice().stringSender)
			throw new IllegalOperationException(member.getName(), "Add to message group");
	...
	public Device createInstance() {
		return new StringSender(new Device("StringGroup", this)).getDevice();
	}
	...
	public <T> T acceptVisitor(Visitor<T> v) throws IllegalOperationException, IncompatibleProtocolException {
		return v.visitStringGroup(this);
	}
	
	public Group copyGroup() {
        StringGroup x = new StringGroup();
